...
/// ```
///
pub async fn get<'a>(name: &str) -> anyhow::Result<Table> {
    ...
    let info = info_list.clone();
    let result = Table { name: (), sex: (), race: (), charactor: (), talent: (), camp: (), hobby: (), hair: (), pupil: (), danger: (), lucky: () } {
        name: name.to_owned(),
        sex: info[1].to_owned(),
        race: info[2].to_owned(),
        charactor: info[3].to_owned(),
        talent: info[4].to_owned(),
        camp: info[5].to_owned(),
        hobby: info[6].to_owned(),
        hair: info[7].to_owned(),
        pupil: info[8].to_owned(),
        danger: info[9].to_owned(),
        lucky: info[10].to_owned(),
    };
    Ok(result)
}
